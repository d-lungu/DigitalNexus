<script>
    ...
    onMount(async () => {
      original = cpuCoolers = await fetchEndpointData(datasetEndpoints.cpucooler);
    });
    ...
    <div class="container flex flex-col mx-auto px-2 items-center justify-center gap-y-2 shadow-xl" style="background:0;">
        ...
        <div class="container flex items-center justify-center">
          <input type="search" bind:value={searchString} on:input={() => {
            cpuCoolers = original.filter((x) => x.name.toLowerCase().includes(searchString.toLowerCase()));
          }} placeholder="Search..." />
        </div>
