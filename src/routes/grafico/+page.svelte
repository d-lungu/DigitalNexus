<script>
    ...
    var chartData = componentNames.map((name) => {
        if (name === 'CPU Cooler') {
            ...
            var price =
                parseFloat(getLocalStorage(`${name.toLowerCase()}Price`)) || 0
        }
        totalCost += price
        return price
    })
...
<div class="container mx-auto">
    <div class="text-5xl text-center py-8">Your Price Chart</div>
    <div class="text-center">
        If you are here it means that you have just built the PC of your dreams
        and that it works perfectly. <br />
        Below you can find the generalized graph of prices as a function of the total
        cost, <br />
        allowing you to observe which component is the most expensive and if you
        want to manage the price distribution better.
    </div>
    ...
    <div class="text-center py-4">
        <a href="/pick-parts"
            ><button class="btn">Return to configuration</button></a
        >
    </div>
</div>
